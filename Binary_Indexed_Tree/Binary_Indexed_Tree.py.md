...
#### 概要
可換群 $G=(G,+,0)$ における一点可算及び区間和の取得を高速に行うことを得意とするデータ構造

#### 使用方法

- `Binary_Indexed_Tree(L, calc, unit, inv)`: $L$ で初期化された Binary Indexed Tree を生成する.
  - `L`: $G$ の元からなるリスト
  - `calc` : $G \times G \to G; (x,y) \mapsto x+y$: 二項演算
  - `unit` : $G$ における単位元
  - `inv` : $G \to G; x \mapsto -x$: $x$ における逆元

以降では $L$ の長さを $N$ とする.

- `get(k)`: 第 $k$ 要素を得る.
- `add(k, x)`: 第 $k$ 要素に $x$ を加え, 更新する.
- `update(k, x)`: 第 $k$ 要素を $x$ に変更し, 更新する.
- `sum(l, r)`: $\displaystyle \sum_{k=l}^r B\[k\]$ を求める.
- `all_sum()`: $\displaystyle \sum_{k=0}^{N-1} B\[k\]=$ `sum(0,N-1)` を求める.
- `binary_search(self, cond)`:
  - $G$ が順序群であることを要求し, しかも $i=0,1, \dots, N-1$ に対して, $B\[i\] \geq 0$ であるとする.
  - また, ${\rm cond}: G \to \\{\mathbb{T}, \mathbb{F} \\}$ は単調増加であるとする.
  - このとき, $\displaystyle {\rm cond} \left(\sum_{i=0}^{k-1} B\[i\] \right)=\mathbb{F}, {\rm cond} \left(\sum_{i=0}^k B\[i\] \right)=\mathbb{T}$ を満たす $0$ 以上 $(N-1)$ 以下の整数 $k$ を求める. ただし, 以下のようなコーナーケースがある.
    - ${\rm cond} (0)=\mathbb{T}$ である場合, $k=-1$ とする.
    - $\displaystyle {\rm cond} \left(\sum_{i=0}^{N-1} 0 \right)=\mathbb{F}$ である場合, $k=N$ とする.
- `B[k]`: `get(k)` と同等
- `B[k]=x`: `update(k, x)` と同等
- `len(B)`: $N$ を返す.
